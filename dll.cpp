...
CList *CMapPlugin::GetCommandListPtr()
{
	return m_CommandList;
}

CList *CMapPlugin::GetGroupListPtr()
{
	return m_GroupList;
}
...
void CMapPlugin::ReadGE64(void *db,CSymbol *ptr)
{
	...
	while(row = (char**)db_fetch_row(result))
	{
		...
		Driver->SetGpsLat(ptr->GetRLat());
		Driver->SetGpsLon(ptr->GetRLon());
		Driver->SetGpsLonMap(ptr->GetRLonMap());		
		Driver->SetGpsLatMap(ptr->GetRLatMap());
