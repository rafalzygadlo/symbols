...
void ExportAlarm(void *db)
{
	...
	wxString export_dir = wxString::Format(_("%s%s"),GetWorkDir(),DEFAULT_EXPORT_DIRECTORY);
	wxMkDir(export_dir);

	wxString fname = wxString::Format(_("%s%s\\alarm_%d_%02d_%02d.csv"),GetWorkDir(),DEFAULT_EXPORT_DIRECTORY,dt.GetYear(),dt.GetMonth(),dt.GetDay());
