...
void CAlarmList::OnContextMenu(wxContextMenuEvent &event)
{
	...
	{
		r = this->GetItemRect(i);
		int a = r.height * (i+1);

		if(event.GetPosition().x < a)
			SetSelection(i);
	}
	
	CMenu *Menu = new CMenu(NULL,_GetUID());
	
	wxMenuItem *item = Menu->Append(wxID_ANY,GetMsg(MSG_NEW));
	Menu->Right(MODULE_ALARM,ACTION_NEW,item);
